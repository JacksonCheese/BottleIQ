...
Owners record only confirmed incoming deliveries, with quantities in units and an expected date. An open delivery due inside the planning window reduces the new order. An overdue open delivery is excluded and holds the recommendation until it is reviewed. On delivery, record actual received units; partial receipts leave the remaining units open. Each receipt is retained as a stock event and adds to estimated on-hand stock after the latest physical inventory snapshot. A newer physical count on or after receipt day supersedes prior receipt adjustments so they are not counted twice. The owner can close a missing/cancelled remainder without adding stock. Same-day CSV counts must be taken after receipt to supersede it correctly; if timing is uncertain, take a fresh count. Existing order drafts do not recalculate; create a new draft after changing incoming stock. Days of supply uses estimated on-hand inventory. Stockout risk projects average demand through lead time and counts confirmed arrivals on their expected dates; an arrival after stock runs out does not hide the risk.
